...
* **Manejo Inteligente de Hardware (Sin Punto Verde):** 
  * A diferencia de otras aplicaciones, HiddenCamera **no** inicializa la cámara al abrir la app. Sólo la activa durante la grabación.
...
* **Guardado Directo en Galería:** Utiliza el paquete `gal` para inyectar transparentemente el archivo `.mp4` resultante a la Galería pública de tu teléfono (en la carpeta **HiddenCamera**) tan pronto terminas de grabar, sin necesitar que mantengas la pantalla encendida.
...
```bash
# Clonar el proyecto
git clone git@github.com:JaimeGDH/hiddenCamera.git
